...
class MqttClient(mqtt.Client):
    ...
    def mqtt_on_connect(self, client: mqtt.Client, userdata, flags, rc):
        for device in self._devices:
            client.subscribe(
                [
                    (self._mqtt_topics["sub"].format(device.name, data_field.name), 0)
    ...
    def mqtt_on_subscribe(self, payload: bytes):
        ...
        self.mqtt_publish_update(
            chosen_device.name, prop_name, chosen_device.get_property(prop_name)
        )
    ...
    def mqtt_publish_update(self, device_name: str, property_name: str, value) -> None:
        if isinstance(value, enum.Enum):
            ...
            payload = str(value)
        self.publish(
            self._mqtt_topics["pub"].format(device_name, property_name),
